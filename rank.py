...
if __name__ == "__main__":
    ...
    # Read bib file
    with open(path2bib, "r") as f:
        lines = f.readlines()

    # Parse out the abstracts
    abstract_regex =  r"(?<=abstractNote=\\{).+\.(?=\\\})"
    seed_abstracts = [re.search(abstract_regex, lines[i]).group(0) if re.search(abstract_regex, lines[i]) else None 
...
#        f.write(f"PAPER WATCH - arXiv - {date.today()}\nTheme : {seed_name}\n\n")
#        for idx in ranking:
#            for v in new_papers[idx].values():
#                f.write(str(v) + "\n")
#            f.write("\n")
